...
combine_files() {
    ...
    for part in "${PARTS[@]}"; do
        ...
        find "${BOOK_DIR}/${part}" -name "*.md" | sort -V | while read -r file;
 do
    ...
    if [ "$include_extras" = "true" ]; then
        ...
        find "b.Philosophical-Lenses" -name "*.md" | sort -V | while read -r file;
 do
            echo -e "\n\n--- ${file} ---\n\n" >> "$dest_file"
            cat "$file" >> "$dest_file"
        done
        ...
        # Append Appendices
        find "c.Appendices" -name "*.md" | sort -V | while read -r file;
 do
            echo -e "\n\n--- ${file} ---\n\n" >> "$dest_file"
            cat "$file" >> "$dest_file"
        done
        ...
        # Append References
        find "d.References" -name "*.md" | sort -V | while read -r file;
 do
            echo -e "\n\n--- ${file} ---\n\n" >> "$dest_file"
            cat "$file" >> "$dest_file"
        done
